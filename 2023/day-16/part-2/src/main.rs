    ...
    while !wall_found && !tile_direction_visited {
        ...
        if visited_tiles_direction.get(&hash_current_conditions) == None {
            ...
            if (current_position.1 == 0 && beaming_direction == BeamingDirection::Left)
                || (current_position.1 == contraption[0].len() - 1
                    && beaming_direction == BeamingDirection::Right)
                || (current_position.0 == 0 && beaming_direction == BeamingDirection::Up)
                || (current_position.0 == contraption.len() - 1
                    && beaming_direction == BeamingDirection::Down)
            {
                wall_found = true;
                // println!("Wall reached");
            } else {
                (current_position, beaming_direction) =
                    select_next_tile(current_position, beaming_direction);
...
fn get_starting_positions_directions(
    contraption: &Vec<Vec<Tile>>,
) -> Vec<(usize, usize, BeamingDirection)> {
    let mut starting_positions_directions: Vec<(usize, usize, BeamingDirection)> = 
        vec![];
    // Top row
    for i in 0..contraption[0].len() {
        starting_positions_directions.push((
            0, i, 
            BeamingDirection::Down
        ));
    }
    ...
    for i in 0..contraption.len() {
        starting_positions_directions.push((
            i, contraption[0].len() - 1, 
            BeamingDirection::Left
        ));
    }
    return starting_positions_directions;
}
...
fn main() {
    ...
    // Calculate the energized tiles from all positions in an edge
    let starting_positions_directions = get_starting_positions_directions(&contraption);
    for starting_position_direction in 
        starting_positions_directions.into_iter().progress() {
        ...
        trace_light_beam(
            &contraption,
            &mut energized_tiles,
            (starting_position_direction.0, starting_position_direction.1),
            starting_position_direction.2,
            &mut 1,
